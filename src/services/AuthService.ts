...
export default class AuthService {
    ...
    async refresh(refreshToken: string): Promise<{accessToken: string}> {
        ...
        const decoded = await this.jwtService.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET as string)
        const isRolesMatched = user.roles.every((role: Role) => decoded.roles.includes(role));
        if (decoded.email !== user.email || !isRolesMatched) {
            throw new HttpException(403, "Forbidden")
        }
